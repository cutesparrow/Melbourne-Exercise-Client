...
struct OnboardingShowView: View {
    ...
    var body: some View {
        let pages = (1...4).map { i in
            AnyView(OnboardingView(viewPage: i))
        }
        var onboardingView =  ConcentricOnboardingView(pages: pages, bgColors: [Color(hex: 0xb598a1),Color(hex: 0x525288),Color(hex: 0xc04851),Color(hex: 0xF9F5ED)],duration: 0.6)
